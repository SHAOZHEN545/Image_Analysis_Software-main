"""Approximations of polylogarithm functions.

Functions that approach several polylogarithms by polynomials.

Precision is on the order of 1e-7 or better. For working with fermions,
the polylog functions ``Li(x)`` are usually used in the form ``-Li(-exp(x))``.
We therefore define functions ``fermi_poly`` as:
``fermi_poly_s(x) := -Li_s(-e^x)``,
with ``Li_s(z)=\sum_{k=1}^{\infty} z^k/k^s``.
This is useful for Fermi statistics. For Bose statistics we define g-functions
in a similar way.
